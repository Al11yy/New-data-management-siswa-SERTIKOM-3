<x-app-layout>
    <!-- Enhanced tahun ajar index with timeline-style cards -->
    <div class="p-6 lg:p-10">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-white tracking-tight">Data Tahun Ajar</h1>
                <p class="text-white/50 mt-1">Kelola periode tahun ajaran dalam sistem</p>
            </div>
            <a href="{{ route('tahun_ajar.create') }}" class="btn-primary px-6 py-3 rounded-xl text-sm flex items-center gap-2.5 w-fit">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse ($tahunAjars as $item)
                <div class="glass-card rounded-3xl p-6 group hover:bg-white/[0.04] transition-all duration-300">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <div x-data="{ open: false }" @click.outside="open = false" class="relative">
                            ...
                            <div x-show="open" x-transition class="absolute right-0 w-44 glass-card-elevated rounded-2xl shadow-2xl z-10 py-2 mt-2">
                                <a href="{{ route('tahun_ajar.edit', $item->id) }}" class="flex items-center gap-3 px-4 py-3 text-sm text-white/70 hover:text-white hover:bg-white/[0.04] transition-all duration-150">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>
                                    Edit
                                </a>
                                <form action="{{ route('tahun_ajar.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus tahun ajar ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <h3 class="text-xl font-semibold text-white mb-2">{{ $item->nama_tahun_ajar }}</h3>
                    <span class="badge badge-info font-mono">{{ $item->kode_tahun_ajar }}</span>
                </div>
            @empty
                <div class="col-span-full">
                    <div class="glass-card rounded-3xl p-12 text-center">
                        ...
                        <p class="text-white/50 mb-4">Belum ada data tahun ajar</p>
                        <a href="{{ route('tahun_ajar.create') }}" class="btn-primary px-5 py-2.5 rounded-xl text-sm inline-flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                            </svg>
