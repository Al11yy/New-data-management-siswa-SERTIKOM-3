    ...
    <div class="page-wrapper">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-bold text-white tracking-tight">Edit Data Siswa</h1>
                <p class="text-white/50 mt-1">Perbarui informasi siswa {{ $siswa->nama_lengkap }}</p>
            </div>
                ...
                <form action="{{ route('siswa.update', $siswa->id) }}" method="POST">
                    ...
                    <div class="mb-10">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="nisn" value="{{ old('nisn', $siswa->nisn) }}" required class="w-full glass-input rounded-xl text-black px-4 py-4 text-sm">
                            </div>
                            <div>
                                <label class="block text-white/70 text-sm font-medium mb-2.5">Nama Lengkap</label>
                                <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $siswa->nama_lengkap) }}" required class="w-full glass-input rounded-xl text-black px-4 py-4 text-sm">
                            </div>
                            <div>
                                <label class="block text-white/70 text-sm font-medium mb-2.5">Jenis Kelamin</label>
                                <select name="jenis_kelamin" required class="w-full glass-input rounded-xl text-white px-4 py-4 text-sm appearance-none cursor-pointer">
                                    <option value="laki-laki" {{ old('jenis_kelamin', $siswa->jenis_kelamin) == 'laki-laki' ? 'selected' : '' }} class="bg-[#141416]">Laki-laki</option>
                                    <option value="perempuan" {{ old('jenis_kelamin', $siswa->jenis_kelamin) == 'perempuan' ? 'selected' : '' }} class="bg-[#141416]">Perempuan</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-white/70 text-sm font-medium mb-2.5">Tanggal Lahir</label>
                                <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $siswa->tanggal_lahir) }}" required class="w-full glass-input rounded-xl text-black px-4 py-4 text-sm">
                            </div>
                        </div>
                    </div>
                            ...
                            <div>
                                ...
                                <select name="jurusan_id" required class="w-full glass-input rounded-xl text-white px-4 py-4 text-sm appearance-none cursor-pointer">
                                    @foreach ($jurusans as $j)
                                        <option value="{{ $j->id }}" {{ old('jurusan_id', $siswa->jurusan_id) == $j->id ? 'selected' : '' }} class="bg-[#141416]">{{ $j->nama_jurusan }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-white/70 text-sm font-medium mb-2.5">Kelas</label>
                                <select name="kelas_id" required class="w-full glass-input rounded-xl text-white px-4 py-4 text-sm appearance-none cursor-pointer">
                                    @foreach ($kelas as $k)
                                        <option value="{{ $k->id }}" {{ old('kelas_id', $siswa->kelas_id) == $k->id ? 'selected' : '' }} class="bg-[#141416]">{{ $k->nama_kelas }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="tahun_ajar_id" required class="w-full glass-input rounded-xl text-black px-4 py-4 text-sm appearance-none cursor-pointer">
                                    @foreach ($tahunAjars as $t)
                                        <option value="{{ $t->id }}" {{ old('tahun_ajar_id', $siswa->tahun_ajar_id) == $t->id ? 'selected' : '' }} class="bg-[#141416]">{{ $t->nama_tahun_ajar }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="mb-10">
                        ...
                        <div>
                            ...
                            <textarea name="alamat" rows="4" required class="w-full glass-input rounded-xl text-white px-4 py-4 text-sm resize-none">{{ old('alamat', $siswa->alamat) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-8 border-t border-white/[0.04]">
                        ...
                        <button type="submit" class="w-full sm:w-auto btn-primary px-8 py-3.5 rounded-xl text-sm">Update Data</button>
                    </div>
                </form>
            </div>
        </div>
