<x-app-layout>
    <div class="p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-extrabold text-white">Data Siswa</h1>
                ...
                <tbody>
                    @foreach ($siswas as $item)
                        <tr class="border-b border-white/10 hover:bg-white/5 transition duration-150">
                            <td class="p-4">{{ $item->nisn }}</td>
                            <td class="p-4">{{ $item->nama_lengkap }}</td>
                            <td class="p-4">{{ $item->jenis_kelamin }}</td>
                            <td class="p-4">{{ $item->jurusan->nama_jurusan }}</td>
                            <td class="p-4">{{ $item->kelas->nama_kelas }}</td>
                            <td class="p-4">{{ $item->tahunAjar->nama_tahun_ajar }}</td>
                            <td class="p-4 text-right">
                                <div x-data="{ open: false }" @click.outside="open = false" class="relative inline-block">
                                    ...
                                    <div x-show="open" x-transition class="absolute right-0 w-40 bg-gray-800/90 backdrop-blur-xl border border-white/20 rounded-lg shadow-lg z-10 @if($loop->last) bottom-full mb-2 @else mt-2 @endif">
                                        <a href="{{ route('siswa.edit', $item->id) }}" class="block px-4 py-2 text-sm text-white hover:bg-white/20 w-full text-left rounded-t-lg">Edit</a>
                                        <form action="{{ route('siswa.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data?')">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <a href="{{ route('siswa.show', $item->id) }}"
                                            class="block px-4 py-2 text-sm text-cyan-300 hover:bg-white/20 w-full text-left">
                                            Detail
                                         </a>
