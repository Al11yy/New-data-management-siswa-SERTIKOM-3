<x-app-layout>
    <div class="p-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-extrabold text-white">Riwayat Kelas Siswa</h1>
            <a href="{{ route('kelas-detail.create') }}"
                ...
                <tbody>
                    @foreach ($details as $item)
                        <tr class="border-b border-white/10 hover:bg-white/5 transition duration-150">
                            <td class="p-4">{{ $item->siswa->nama_lengkap }}</td>
                            ...
                            <td class="p-4 text-right">
                                <div x-data="{ open: false }" @click.outside="open = false" class="relative inline-block">
                                    ...
                                    <div x-show="open" x-transition class="absolute right-0 w-40 bg-gray-800/90 backdrop-blur-xl border border-white/20 rounded-lg shadow-lg z-10 @if($loop->last) bottom-full mb-2 @else mt-2 @endif">
                                        <a href="{{ route('kelas-detail.edit', $item->id) }}" class="block px-4 py-2 text-sm text-white hover:bg-white/20 w-full text-left rounded-t-lg">Edit</a>
                                        <form action="{{ route('kelas-detail.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data?')">
                                            @csrf
                                            @method('DELETE')
